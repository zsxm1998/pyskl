...
train_pipeline = [
    dict(type='AllFrames', num_clips=1),#dict(type='UniformSampleFrames', clip_len=48),
...
val_pipeline = [
    dict(type='AllFrames', num_clips=1),#dict(type='UniformSampleFrames', clip_len=48, num_clips=1),
    dict(type='PoseDecode'),
    dict(type='PoseCompact', hw_ratio=1., allow_imgpad=True),
...
test_pipeline = [
    dict(type='AllFrames', num_clips=1),#dict(type='UniformSampleFrames', clip_len=48, num_clips=10),
    dict(type='PoseDecode'),
    dict(type='PoseCompact', hw_ratio=1., allow_imgpad=True),
    dict(type='Resize', scale=(64, 64), keep_ratio=False),
    dict(type='GeneratePoseTarget', with_kp=False, with_limb=True, skeletons=skeletons,
...
log_level = 'INFO'
work_dir = './work_dirs/e3v/posec3d/c3d_limb'
auto_resume = False
